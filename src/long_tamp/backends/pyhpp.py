...
class PyHPPBackend(BackendBase):
    ...
    def random_config(self, max_attempts: int = 1000) -> np.ndarray:
        ...
        (`long_tamp.planning.config.ConfigGenerator.max_attempts`).
    ...
    def load_robot(
        ...
        baked into the URDF (as a combined multi-robot URDF might do).
        """
    ...
    def play_path_vector_with_viz(
        ...
        if visualizer is not None and graph_builder is not None:
            try:
                from long_tamp.visualization.live_graph_viz import (
                    LivePathPlayer,
                )
    ...
    def play_and_record_path(
        ...
        if self.viewer is None:
            self.visualize()
        from long_tamp.visualization.video_recorder import VideoRecorder
