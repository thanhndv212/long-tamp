#!/usr/bin/env python3
"""long_tamp.planning.constraints
...
try:
    from long_tamp.utils import xyzquat_to_se3
except ImportError:
    xyzquat_to_se3 = None
    ...
    def register_from_defs(
        ...
        for ctype, user_name, args in constraint_defs:
            if ctype == "grasp":
                ...
                mask = args.get("mask")
                factory_name = self.register_grasp(gripper, handle, transform, mask)
                ...
                factory_name = self.register_placement(obj_name, transform, mask)
                name_map[user_name] = factory_name
