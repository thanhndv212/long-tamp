...
from long_tamp.config.base_config import Defaults
...
def _yaml_with_grasp_goals(cls, goal_states):
    ...
    Mirrors the logic a task's own ``with_grasp_goals`` classmethod would
    implement on its ``TaskConfigurations``.
...
class YamlTaskLoader:
    ...
    The YAML schema is documented in ``script/twin/config/twin_lift_ball_config.yaml``.
