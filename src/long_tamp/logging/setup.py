"""
Python ``logging`` module configuration for long_tamp.
...
These are completely optional.  The :class:`~long_tamp.logging.RunLogger`
...
def configure_logging(
    ...
    """Configure the ``long_tamp`` root logger.
    ...
    Returns:
        The configured ``logging.Logger`` instance for ``"long_tamp"``.
    """
...
def get_logger(name: Optional[str] = None) -> logging.Logger:
    """Return a logger under the ``long_tamp`` namespace.
    ...
    """
    if name:
        return logging.getLogger(f"long_tamp.{name}")
    return logging.getLogger("long_tamp")
