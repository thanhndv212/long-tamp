...
committed to THIS candidate target" -- while searching for a phase-2
grasp that also leaves a phase-3 grasp reachable (see that method's
docstring for the full story). It must never call
...
GRIPPERS = ["g_arm1_tool", "g_arm2_wb", "g_arm3_part"]
HANDLES = ["h_tool", "h_wb", "h_con0"]
...
def test_mutating_the_copy_does_not_affect_the_original():
    ...
    probe = original.copy()
    probe.update_grasp("g_arm2_wb", "h_wb")
    probe.update_grasp("g_arm3_part", "h_con0")

    assert original.current_grasps == original_snapshot
    assert original.current_grasps["g_arm2_wb"] is None
    assert original.current_grasps["g_arm3_part"] is None
    # The copy itself did pick up the mutations.
    assert probe.current_grasps["g_arm2_wb"] == "h_wb"
    assert probe.current_grasps["g_arm3_part"] == "h_con0"
...
def test_mutating_the_original_after_copying_does_not_affect_the_copy():
    ...
    original.update_grasp("g_arm1_tool", "h_tool")

    assert probe.current_grasps["g_arm1_tool"] is None
