> **Legacy.** Written when this library still had a CORBA backend alongside PyHPP and the
> SpaceLab screwdriving mission was the in-repo flagship example — both have since been
> removed. References to `backends/corba.py` and `script/spacelab/` describe a tree that
> no longer exists here. Kept for historical engineering rationale, not as a live plan.
...
`src/long_tamp` (490 total) ranked by line count and max
...
(`/Users/thanhndv212/Develop/agimus-ws/long_tamp` →
`.../src/long_tamp` in-container) — branch switches and edits are
...
2. **The installed `long_tamp` under `PYTHONPATH` is a static CMake
   install copy**, not the live source — editing `src/long_tamp/*.py`
   does **not** change what `import long_tamp` resolves to by
...
docker exec hpp-agimus-arm64 bash -lc '
  ...
  export PYTHONPATH=/home/thanhndv212/devel/hpp/src/long_tamp/src:$PYTHONPATH &&
  cd /home/thanhndv212/devel/hpp/src/long_tamp &&
...
## Findings: size/complexity hotspots (`src/long_tamp`)
...
fixes already landed in source: the Python-side retry loop (long_tamp)
...
- **Correction found during Phase 2 review (2026-08-08), superseding the
  ...
  *anything* from `long_tamp`, even a pure-Python helper with zero
  HPP calls, transitively requires `pyhpp` on host, because
  `long_tamp/__init__.py` unconditionally does `from .planning
...
- `print()`-vs-structured-logging migration — ✅ **Done 2026-08-09**.
  ...
  `logging.Logger` calls via `long_tamp.logging.get_logger()`, with
