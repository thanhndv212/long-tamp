...
## 4. `SpacelabPlannerNode` (`long_tamp_ros`)
...
since been removed from `long_tamp` — any launch file still passing `backend:=corba`
needs updating.)
...
3. **`attach_service_node`** (Python, `long_tamp_ros`) —
...
```bash
ros2 launch long_tamp_ros spacelab_ur10_gz_full_test.launch.py backend:=pyhpp
```
...
```bash
ros2 service call /spacelab_planner_node/plan_grasp long_tamp_interfaces/srv/PlanGrasp \
...
```bash
python3 src/long_tamp_ros/scripts/test_ur10_gz.py
```
...
If a DBT mission needs one of the "No" rows, that's new work on `long_tamp_ros` — extend
...
`long_tamp_ros/long_tamp_ros/gui/_sequence_controller.py::_send_direct_phase()`:
