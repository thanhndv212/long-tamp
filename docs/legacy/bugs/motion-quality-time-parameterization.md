> **Legacy.** Written when this library still had a CORBA backend alongside
> PyHPP; several sections compare the two. The CORBA backend has since been
> removed — PyHPP is now the only backend. The root-cause analysis and the
> resulting PyHPP defaults are still current; kept here for historical
> reference rather than folded into live docs.
...
### `src/long_tamp/backends/pyhpp.py`
...
### `src/long_tamp/backends/corba.py`
...
### `src/long_tamp/config/base_config.py`
...
### `src/long_tamp/config/yaml_loader.py`
...
### `src/long_tamp/tasks/grasp_sequence.py`
...
A task's own config module/YAML — added `TIME_PARAM_SAFETY`,
`TIME_PARAM_ORDER` class attributes, exposed under `optimization:` in YAML:
```yaml
optimization:
  time_param_safety: 0.5    # half speed (0.95 is the backend default)
  time_param_order: 2
```
...
This is the primary lever.  Edit your task's config YAML:
...
To change it at runtime without editing the YAML:
...
loader = YamlTaskLoader("path/to/your_task_config.yaml")
