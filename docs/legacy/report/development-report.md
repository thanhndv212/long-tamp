> **Legacy.** Written when this library still had a CORBA backend alongside PyHPP, and
> when the SpaceLab screwdriving mission was the in-repo flagship example — both have
> since been removed (CORBA entirely; SpaceLab onto a private validation branch). Sections
> comparing the two backends, and examples referencing `script/spacelab/`, describe a
> tree that no longer exists here. Kept for historical engineering rationale (why certain
> defaults and design decisions were made) rather than as a live doc — see
> [`standalone-usage.md`](../../usage/standalone-usage.md) for current usage.

# long_tamp — Development & Engineering Report

How a task/orchestration layer built on top of HPP (Humanoid Path Planner) turns a
high-level, multi-phase assembly goal into one collision-free motion for several robot
arms — the decisions that made it faster and more reliable than planning against bare
HPP, and a guide to building a mission with it.
...
constraint graph and one start/goal pair, find a path. `long_tamp` is the layer above
...
repository modifies HPP's planning algorithms. What `long_tamp` contributes sits on
...
| 2026-08-01 – 09 | **Production hardening, round one.** Five stacked wall-clock-hang bugs found and fixed across `long_tamp` and upstream `hpp-core` (§04.4); the constraint-graph-factory memoization defect found and fixed in `hpp-python` (§04.3); a disciplined, container-verified refactor of the 4,500-line `GraspSequencePlanner` (§04.10). |
...
flowchart TB
    ...
    script -.uses.-> cli
```
*fig. 1 — module layering, `src/long_tamp`. Dependency direction is strictly downward.*
...
*Source: `ARCHITECTURE.md` "Design goals"; `src/long_tamp/planning/sequential_graph_factory.py`, `sequential_grasp_filter.py`.*
...
the same process as the caller. `long_tamp` defaults every task to PyHPP and keeps
...
*Source: `ARCHITECTURE.md` "Configuration"; `src/long_tamp/config/yaml_loader.py`.*
...
`long_tamp` has two dependency tiers, installed in order — the native bindings first,
...
`/tmp/long_tamp/<task_slug>_<timestamp>/`.

```python
from long_tamp.logging import print_run_summary, get_replay_config

print_run_summary("/tmp/long_tamp/my_pick_.../run_....jsonl")
cfg = get_replay_config("/tmp/long_tamp/my_pick_.../run_....jsonl")  # reproduce the run
```
...
Importing anything from `long_tamp` transitively imports `pyhpp`, so the test suite
...
Six defects, spanning `long_tamp` itself and two upstream packages (`hpp-core`,
...
<details>
<summary><b>Five stacked missing wall-clock bounds cause indefinite planning hangs</b> — <code>long_tamp / hpp-core</code>, critical</summary>
...
<details>
<summary><b>Motion quality: unset time-parameterization safety factor and wrong optimizer order</b> — <code>long_tamp</code>, medium</summary>
...
<details>
<summary><b>PyHPP factory mode: three interacting <code>LockedJoint</code> bugs, 100% grasp-edge failure</b> — <code>long_tamp / hpp-python</code>, high</summary>
...
<details>
<summary><b><code>ConstraintGraphFactory</code> back-reference cycle leaks ~235 MB per phase-graph build</b> — <code>long_tamp</code>, medium</summary>
...
present at `src/long_tamp/planning/graph.py:1226`. Distinct from the algorithmic
...
| Area | Lines | Note |
|---|---:|---|
| `src/long_tamp/`, total | 22,363 | library code |
...
Compiled from `long_tamp`'s own documentation set — `README.md`, `ARCHITECTURE.md`,
